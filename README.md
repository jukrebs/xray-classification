...
Create and activate a virtual environment (conda, venv, etc.), then install dependencies:
...
Alternatively, you can rely solely on `pyproject.toml`:

```bash
pip install .
```

Make sure environment variables for data and W&B are configured, e.g.:
...
### 6.1. Local Federated Simulation

You can run a local Flower simulation using the configuration in `pyproject.toml`:
...
This will start:
- One server process running `berlin25_xray.server_app`.
- Three virtual supernodes/clients, one per hospital.

### 6.2. Cluster / SuperNode Deployment

For the hackathon cluster, `pyproject.toml` also defines a `cluster` and `supergrid-deployment` federation.  
Exact commands depend on the cluster environment, but typically look like:

```bash
flwr run . --federation cluster
```

The server and client apps are then orchestrated by the Flower infrastructure.

### 6.3. Local Single‑Hospital Debugging

For quick local experiments without federated orchestration, use:
...
This script:
- Trains the `Net` model on one hospital’s data for a few epochs.
- Prints loss and metrics after each epoch.
- Saves a local `.pt` checkpoint.

You can change `HOSPITAL`, `EPOCHS`, `LEARNING_RATE`, etc. inside `local_train.py`.

### 6.4. Offline Evaluation of a Saved Model

To evaluate a saved model artifact from W&B on all available splits:

1. Set `WANDB_MODEL_PATH` in `evaluate.py` to point to your artifact, e.g.:
   ```python
   WANDB_MODEL_PATH = "your-entity/your-project/your-artifact:version"
   ```
2. Run:
   ```bash
   python evaluate.py
...
This script:
- Downloads the model from W&B.
- Evaluates ROC‑AUC on:
  - `HospitalA/B/C` eval splits.
  - Test A/B/C and OOD test D (if present).
- Prints per‑split AUROC and a patient‑weighted average across hospitals.

---

## 7. What’s Special About This Solution?
...
- Built on top of the Flower federated learning framework.
- Uses PyTorch, torchvision, Hugging Face `datasets`, and Weights & Biases for training, data handling, and experiment tracking.
